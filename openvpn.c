...
static int _access_config_file() {
...
char *call_command(const char *command) {
...
void _free_multiline(char ***lines) {
...
char **_call_command_multiline(const char *command) {
    if (_open_mgmt_socket())
        return NULL;
    _flush();
    if (!_writeline(command))
...
static int _stop_openvpn(void) {
    ...
    nakd_log(L_INFO, "Sending SIGTERM to OpenVPN, PID %d", _openvpn_pid);

    /* Causes OpenVPN to exit gracefully. -- OpenVPN manpage */
    int result = kill(_openvpn_pid, SIGTERM);
    if (result == -1) {
        nakd_log(L_WARNING, "Couldn't send SIGTERM to OpenVPN, PID %d: ",
                                          _openvpn_pid, strerror(errno));
        return -1;
    }
...
static int _restart_openvpn(void) {
    nakd_log(L_INFO, "Sending SIGHUP to OpenVPN, PID %d", _openvpn_pid);
     ...
     */
    int result = kill(_openvpn_pid, SIGHUP);
    if (result == -1) {
        nakd_log(L_WARNING, "Couldn't send SIGHUP to OpenVPN, PID %d: ",
                                         _openvpn_pid, strerror(errno));
    }
